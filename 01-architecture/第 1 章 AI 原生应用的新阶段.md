...
因此，在 Harness 与硬件之间正浮现一个尚无清晰名字的层次：它不持有任务语义，却为所有 Agent 提供公共的运行对象、能力接入、可强制的边界与统一的证据来源。本书第 30 章称之为 Agentic OS。
...
这也对应 1.4.2 提出的五个架构问题：如何设计架构与如何构建，主要关乎形态选择与 Harness 的能力组合；如何运行、如何治理、如何调优，则大量落在这一公共层上。第 2 章先给出完整参考架构，第 30 章再回到这一层次，讨论它的雏形、边界与仍然开放的问题。
